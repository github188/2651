...
#include "avi_encoder_app.h"   // 它會�?event �?UVC, UAC �?QUEUE
...
static void state_usb_isr(void)
{
    ...
    {
        //drv_l1_usb_soft_disconnect();  // �?D+, D- 壓下去，不丟信號給PC
...
void state_usb_entry(void *para1)
{
    ...
    while (1)
    {
        ...
        case MSG_USBCAM_PLUG_OUT:
            ...
            usb_uvc_task_del();		// 也許 Video 還繼續送過來，所以要�?stop 後再結束
            usb_uac_task_del();
            time_out = 0;
            usb_status = USB_EXIT;
