...
static void dma_isr(void)							// Device ISR
{
    ...
    if (pending & C_DMA4_INT_PEND)  		// DMA4 interrupt is pending
    {
        ...
        {
            if (pending & C_DMA4_TIMEOUT)   	// DMA transmit timeout
            {
                if (dma_notify_variable[C_DMA_CH4])
                {
                    *((INT8S *) dma_notify_variable[C_DMA_CH4]) = (INT8S) C_DMA_STATUS_TIMEOUT;
                }
#if _OPERATING_SYSTEM != _OS_NONE
            ...
            else
            {
                if (dma_notify_variable[C_DMA_CH4])
                {
                    *((INT8S *) dma_notify_variable[C_DMA_CH4]) = (INT8S) C_DMA_STATUS_DONE;
                }
#if _OPERATING_SYSTEM != _OS_NONE
                if (dma_notify_queue[C_DMA_CH4])
                {
    ...
    if (pending & C_DMA5_INT_PEND)  		// DMA5 interrupt is pending
    {
        ...
        {
            if (pending & C_DMA5_TIMEOUT)   	// DMA transmit timeout
            {
                if (dma_notify_variable[C_DMA_CH5])
                {
                    *((INT8S *) dma_notify_variable[C_DMA_CH5]) = (INT8S) C_DMA_STATUS_TIMEOUT;
                }
#if _OPERATING_SYSTEM != _OS_NONE
            ...
            else
            {
                if (dma_notify_variable[C_DMA_CH5])
                {
                    *((INT8S *) dma_notify_variable[C_DMA_CH5]) = (INT8S) C_DMA_STATUS_DONE;
                }
#if _OPERATING_SYSTEM != _OS_NONE
                if (dma_notify_queue[C_DMA_CH5])
                {
    ...
    if (pending & C_DMA6_INT_PEND)  		// DMA6 interrupt is pending
    {
        ...
        {
            if (pending & C_DMA6_TIMEOUT)   	// DMA transmit timeout
            {
                if (dma_notify_variable[C_DMA_CH6])
                {
                    *((INT8S *) dma_notify_variable[C_DMA_CH6]) = (INT8S) C_DMA_STATUS_TIMEOUT;
                }
#if _OPERATING_SYSTEM != _OS_NONE
            ...
            else
            {
                if (dma_notify_variable[C_DMA_CH6])
                {
                    *((INT8S *) dma_notify_variable[C_DMA_CH6]) = (INT8S) C_DMA_STATUS_DONE;
                }
#if _OPERATING_SYSTEM != _OS_NONE
                if (dma_notify_queue[C_DMA_CH6])
                {
    ...
    if (pending & C_DMA7_INT_PEND)  		// DMA7 interrupt is pending
    {
        ...
        {
            if (pending & C_DMA7_TIMEOUT)   	// DMA transmit timeout
            {
                if (dma_notify_variable[C_DMA_CH7])
                {
                    *((INT8S *) dma_notify_variable[C_DMA_CH7]) = (INT8S) C_DMA_STATUS_TIMEOUT;
                }
#if _OPERATING_SYSTEM != _OS_NONE
            ...
            else
            {
                if (dma_notify_variable[C_DMA_CH7])
                {
                    *((INT8S *) dma_notify_variable[C_DMA_CH7]) = (INT8S) C_DMA_STATUS_DONE;
                }
#if _OPERATING_SYSTEM != _OS_NONE
                if (dma_notify_queue[C_DMA_CH7])
                {
