...
const INT16U Tbl_TV_RegSet[][9] =
{
    ...
    //	  {0x503E,0x5A00,0x8200,0x0000,0x6D4E,0x1F10,0x7C00,0xF007,0x2100},  //0: NTSC-M
    {0x815E, 0x5A00, 0x7600, 0x0000, 0x6D4E, 0x1F00, 0x7C00, 0xF003, 0x2100}, //1: NTSC-J
    //	  {0x503E,0x5A00,0x7600,0x0000,0x6D4E,0x1F00,0x7C00,0xF007,0x2100},  //1: NTSC-J
    ...
    //	  {0x503E,0x5A00,0x8200,0x0000,0x6D4E,0xA340,0xEF00,0xE607,0x2100},  //3: PAL-M
    ...
    //	  {0x503E,0x5A00,0x7600,0x0000,0x6D4E,0xB800,0x1E00,0xF005,0x2100},  //7: NTSC-J (Non-Interlaced)
...
#define PPU_FRAME_BASE_MODE_COPY  (1<<7)

    reg = PPU_FRAME_BASE_MODE_COPY;
    reg |= FB_FORMAT << 8;
    reg |= FB_MONO << 10;
    reg |= YUV_TYPE << 20;
    reg |= (1 << 24); // TFTLB
