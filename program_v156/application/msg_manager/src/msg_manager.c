...
#if 1
void msg_Q_lock(MSG_Q_ID msgQId)
{
    INT8U err;

    OSSemPend(msgQId->msg_Q_sem, 0, &err);
}

void msg_Q_unlock(MSG_Q_ID msgQId)
{
    OSSemPost(msgQId->msg_Q_sem);
}
#endif
...
#if 1
    if(msgQId->msg_Q_sem == NULL)
    {
        msgQId->msg_Q_sem = OSSemCreate(1);
    }
#endif
...
void msgQDelete (MSG_Q_ID msgQId)
{
    ...
    OSSemDel(msgQId->msg_Q_sem, 0, &err);
    gp_free((void *)msgQId);
}
...
#endif
        DBG_PRINT("over max message length\r\n");
        while(1);
        return -1;
    }
