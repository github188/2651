...
void ap_video_record_reply_action(STOR_SERV_FILEINFO * file_info_ptr)
{
	...
	//INT8U temp[6] = {0, 1, 2, 3, 5, 10};			//modified by zhibo, 17/05/26
	INT8U			temp[9] =
	{
		0, 1, 2, 3, 5, 7, 10, 15, 20
	};

	//7,10,15,	 Liuxi modified in 2015-01-17,
	ap_video_record_sts_set(~VIDEO_RECORD_WAIT);
...
void video_calculate_left_recording_time_enable(void)
{
	...
	if((my_pAviEncVidPara->encode_width == AVI_WIDTH_1080FHD) || (my_pAviEncVidPara->encode_width == AVI_WIDTH_1080P))
	 {
		left_recording_time = freespace / (3*1024*1024);	//estimate one frame = 100KB, so one second video takes 100KB
	*30 = 3MB
	} else if (my_pAviEncVidPara->encode_width == AVI_WIDTH_720P) {
		left_recording_time = freespace / (2458*1024);		//estimate one frame = 80KB, so one second video takes 80KB*30
	 = 2.4MB
	} else {
		left_recording_time = freespace / (1843*1024);	//estimate one frame = 60KB, so one second video takes 60KB*30 
	= 1.8MB
	}
