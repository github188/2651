...
static INT8U  short_key_flag = 0;      //DV与DC同时接收信号时, 防止重复发功能指令
...
static INT32U FilkerLedCnt = 0;
static INT32U FilkerLedTime = 0;
	...
	switch((INT32U)mode)
		{
			...
			case LED_USB_CONNECT:	
			    ...
			    g_led_g_state = 2;	//程序或时间更新完毕，两个灯同时闪烁
			    g_led_r_state = 2;
			    g_led_flicker_state = 0;
				DBG_PRINT("led_type = LED_UPDATE_FINISH\r\n");
			break;
...
{	
   INT8U type=NULL;
   static INT8U bFilerLedFlag = 0;
   static INT32U IsrLedCnt = 0;
   static INT32U IsrLedTime = 0;
	//if(card_space_less_flag)
	//	return;
	
	if (FilkerLedCnt)
	{
		if (++IsrLedTime >= FilkerLedTime)
		{
			IsrLedTime = 0;
			bFilerLedFlag ^= 1;
			if (bFilerLedFlag) 
	...
	if (type == GENERAL_KEY) 
		{
			 ...
			 key_map[1].key_function = (KEYFUNC) ap_peripheral_null_key_exe;
...
void ap_peripheral_capture_key_exe(INT16U *tick_cnt_ptr)
{
	...
	if (!s_usbd_pin) 
	{
		/*
		#if SUPPORT_LONGKEY == CUSTOM_ON
		if(*tick_cnt_ptr >= DC_LongSingle_width_900)
		{//大于898  无效
			*tick_cnt_ptr = 0;
		}
		else if((*tick_cnt_ptr >= DC_LongSingle_width_700)&&(*tick_cnt_ptr < DC_LongSingle_width_900))
		{ //电量4格 703-898(飞控: 800ms输出)
  			//ap_state_handling_icon_clear_cmd(ICON_BATTERY_0,ICON_BATTERY_1,ICON_BATTERY_2);
        	//ap_state_handling_icon_show_cmd(ICON_BATTERY_3,NULL,NULL);
		}
		else if((*tick_cnt_ptr >= DC_LongSingle_width_450)&&(*tick_cnt_ptr < DC_LongSingle_width_700))
		{//低电 453-703ms (飞控: 600ms输出)
		...
		else if((*tick_cnt_ptr >= LongSingle_width_200)&&(*tick_cnt_ptr < DC_LongSingle_width_450))
		{//拍照 203-453ms (飞控: 300ms输出)
			if((ap_state_handling_storage_id_get()!= NO_STORAGE)&&(!pic_down_flag)&&(!card_space_less_flag)&&(!video_down_flag))
			{
		...
		#endif
	  	{//电量2格 47-203ms (飞控: 100ms输出)
	  		if ((video_record_sts & 0x2) == 0)
	  		{//非录像状态
	  			//ap_state_handling_icon_clear_cmd(ICON_BATTERY_0,ICON_BATTERY_2,ICON_BATTERY_3);
        		//ap_state_handling_icon_show_cmd(ICON_BATTERY_1,NULL,NULL);
        	}
        }
        */
