...
INT32S system_clk_init(void)
{
	...
	R_SYSTEM_LVR_CTRL = 0x20;				// 設成 1.2V， CorePower 1.32V 用外灌
...
{
	INT32U SysClk, SpiClk, SpiDelay, SDramPara;
	...
	MHZ	  = clk_mhz;
	system_clk_alter(SysClk, SpiClk, SpiDelay, SDramPara);
...
INT32S sys_msdc_clk_active(void)
{
	...
	#endif
	// ==========================
	timer_pwm_setup(TIMER_B, (12000000), 0, PWM_NRO_OUTPUT);
	R_SYSTEM_CLK_EN0 &= ~0x4840; //disable cdsp, disable csi, disable dac
	{ // uart
...
INT32S sys_msdc_clk_restore(void)
{
	...
	R_SYSTEM_CLK_EN0 |= 0x4840; //enable cdsp
	...
	{ // SD
		if (drvl2_sd_init()==0) {
			drvl2_sd_bus_clock_set(16000000);///(50000000);   // set sd clk 12MHz
		}
		else {
			DBG_PRINT("SD Initial Fail\r\n");
		}
