...
*/
#define DENOISE_SELECT 		BOTH_DENOISE
...
static void gp_cdsp_handle_eof(void)
{
	...
	{	
		...
		{
			hwCdsp_SetBadPixel(80, 80, 80);////cvdfdt
					...
					*/
					...
					if((ret == AWB_FAIL) && ((CdspDev->ae_awb_flag & CDSP_LOW_LUM) != 0) && (lum <= TARGET_Y_LOW))
					{
						...
						if(CdspDev->awb_low_lum_cnt >= 3)
						{
							...
							CdspDev->awb_low_lum_cnt = 0x80000000;//-512;
						}
					}
...
void gp_isp_start(INT32U dummy_addr,INT32U gpSENSOR_WIDTH, INT32U gpSENSOR_HEIGHT)	
{
	...
	//hwCdsp_IntplThrSet(24, 240);	//GP15B
	...
	hwFront_SetFrameSize(0x170, 0x02, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	//BGGR    Liuxi modified in 2015-01-21
	#else
	...
	hwFront_SetFrameSize(0x170, 0x02, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	//BGGR     Liuxi modified in 2015-01-21
	//hwFront_SetFrameSize(0x08, 0x01, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);
