...
void ap_state_handling_connect_to_pc(INT32U prev_state)
{
	...
	#else
	OSTimeDly(70); //检测USB还是充电器插入时会init一次 USB（主要是检测PC传来的Reset信号），下面进入USBD的时候还会init一次，两次init间隔不能太近，否则USB硬体会有问题
	temp = 0;
	#endif
	
	nRet = _devicemount(MINI_DVR_STORAGE_TYPE); //检测TF卡
	...
	if (nRet >= 0) temp = 0;   //有卡进入USB模式
	else temp = 1;			   //无卡进入PC CAMERA模式
