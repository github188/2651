...
//INT16S time_stamp_position_x, time_stamp_position_y,video_osd_position_y;
...
void ap_state_resource_OSD_font_color_set(INT16U color);
INT16U ap_state_resource_OSD_font_color_get(void);
extern void ap_state_resource_OSD_video_position_y_set(INT16S y);
extern INT16S ap_state_resource_OSD_video_position_y_get(void);
		...
		for (i=str_info->font_offset_h,buf_h_idx = 0; i<font.font_height; i++,buf_h_idx++) {
			...
			if (type == YUV420_DRAW) {
				/*
				 ...
				 计: ﹚玡兵Y2*width bytes
				 案计:钡铬奔1.5*width bytes  
				*/
...
{
	INT32U offset_pixel, font_data, offset_tmp, byte_tmp, len, j, width, height,k; //offset, 
	INT32S i, buf_h_idx;
	const INT8U *input_buffer;
	//t_FONT_STRUCT font_header; //lx
	t_FONT_TABLE_STRUCT font;	
	...
	if (target_char == 0x2E)
	{
		input_buffer = FontArialNarrow_ZK[10]+str_info->font_offset_h*byte_tmp;  /* . */
	}
	else if (target_char == 0x2F)
	{
		input_buffer = FontArialNarrow_ZK[11]+str_info->font_offset_h*byte_tmp;  /* / */
	}
	else if (target_char == 0x3A)
	{
		input_buffer = FontArialNarrow_ZK[12]+str_info->font_offset_h*byte_tmp;  /* : */
	}  
	else if (target_char == 0x76)
	{
		input_buffer = FontArialNarrow_ZK[13]+str_info->font_offset_h*byte_tmp;  /* v */
	}
	else
	{
		target_char -= 0x30;
		input_buffer = FontArialNarrow_ZK[target_char]+str_info->font_offset_h*byte_tmp;  /* 数字 */
	}
	//下面两行采用完整的ACSII字库时使用
	//target_char -= 32;    
	//input_buffer = FontArialNarrow_ZK[target_char]+str_info->font_offset_h*byte_tmp;
	//-------------------------------------------------------------------------------------
	/*
	if(target_char==0x2F)
	{
		...
		for(k=0;k<font.bytes_per_line;k++)
		{
			...
			for (j=0 ; j<len ; j++) {
				...
				if (font_data & 0x80) {
					...
					if (type == YUV420_DRAW) {
						//*((INT8U *)frame_buff + offset_pixel) = 0xFF;
						*((INT8U *)frame_buff + offset_pixel) = str_info->font_color;
					} else {
						*(frame_buff + offset_pixel) = str_info->font_color;
					}
				}
