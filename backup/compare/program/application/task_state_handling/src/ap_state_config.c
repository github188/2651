...
static void ap_state_resource_time_stamp_position_x_set(INT16S x);
static void ap_state_resource_time_stamp_position_y_set(INT16S y);
static void ap_state_resource_OSD_video_position_y_set(INT16S y);
static void ap_state_resource_OSD_font_color_set(INT16U color);

//extern void Time_card_storage(void);
static INT16S	time_stamp_position_x, time_stamp_position_y, video_osd_position_y;
static INT16U	osd_color;
...
void ap_state_config_default_set(void)
{
	...
	ap_state_config_capture_date_stamp_set(CAPTURE_OSD_SHOW); //设置为1表示拍照时显示水印，为0表示拍照时不显示水印
	ap_state_config_date_stamp_set(VIDEO_OSD_SHOW); //设置为1表示录像时显示水印，为0表示录像时不显示水印

	ap_state_resource_time_stamp_position_x_set(TIME_OSD_POS_X); //65
	ap_state_resource_time_stamp_position_y_set(CAPTURE_TIME_OSD_POS_Y); //
	ap_state_resource_OSD_video_position_y_set(VIDEO_TIME_OSD_POS_Y); ////video_y: 3-44, 越大越靠上								
	ap_state_resource_OSD_font_color_set(TIME_OSD_COLOR); //
...
static void ap_state_resource_OSD_video_position_y_set(INT16S y)
{
	video_osd_position_y = y;
}


INT16S ap_state_resource_OSD_video_position_y_get(void)
{
	return video_osd_position_y;
}
