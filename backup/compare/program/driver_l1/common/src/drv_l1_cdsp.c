...
void MipiIF_Setting( unsigned char mipi_type, unsigned int width, unsigned int height)	/* 0: Raw 1:YUV */
{
	...
	/*set Mclk(CSI_CLKO) IO_POS to IOD7*/
	(*(volatile unsigned *)0xC000010C) |= 0x21;		//for MIPI Mclk
	
	/*set Mclk(CSI_CLKO) to IOD12*/
	//(*(volatile unsigned *)0xC000010C) |= 0x42;		//for MIPI Mclk
	
	(*(volatile unsigned *)0xD000001C) |= 0x08;		//Enable Mclk
	
	(*(volatile unsigned *)0xD000000C) |= 0x4000;	//MCLK/2

	//R_CSI_TG_CTRL1 |= 0x00000080;			/*0xD0500244,//Enable CSI MCLK from osc(6/12/27MHz)*/
	
	R_CSI_TG_CTRL1 |= 0x0000880;			/*0xD0500244,//Enable CSI MCLK from "SystemCLK/4 = 96/4"*/
	
	//rCDSP_YUV_MODE |= (2<<2);  				//sel source path	/*D00000008[3:2]	0:From DRAM 1:Sen :2 MIPI*/
	(*(volatile unsigned *)0xD0000008) |= (2<<2);	//sel source path	/*D00000008[3:2]	0:From DRAM 1:Sen :2 MIPI*/
	
	if(mipi_type == C_YUV_FMT) 	{			/*D00000008[1] sel YUV/ RAW mode*/
		(*(volatile unsigned *)0xD0000008) |= (1<<1) ;		 	//1: Run YUV mode //cdsp clock sel, yuyv 2pixel clock
 	}
	else {
		(*(volatile unsigned *)0xD0000008) &= ~(1<<1);			//0: Run raw mode, raw 1pixel clock
	}	 
	...
	if(mipi_type == C_YUV_FMT) {      
		(*(volatile unsigned *)0xD0F00014) |= (height & 0xFFFF) << 16 | ((width << 1) & 0xFFFF);//0x01e00500; //P_IMAGE_SIZE
	}
	else {
		(*(volatile unsigned *)0xD0F00014) |= (height & 0xFFFF) << 16 | (width & 0xFFFF);//0x01e00280;
	}
